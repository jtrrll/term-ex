...
func TestGetSurrounding(t *testing.T) {
	for _, inputs := range [][4]int{
		...
		assert.False(t, actual.Contains(position))
		assert.Equal(t, inputs[3], actual.Cardinality())
	}
}
...
func FuzzGetSurrounding(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, x int64, y int64, radius uint64) {
		...
		assert.False(t, actual.Contains(center))
		assert.LessOrEqual(t, actual.Cardinality(), int(math.Pi*math.Pow(float64(radius)+math.Sqrt2, 2)))
		for v := range actual.Iter() {
...
func TestDistanceTo(t *testing.T) {
	for _, inputs := range [][5]int64{
		...
		assert.Equal(t, float64(inputs[4]), p1.DistanceTo(p2))
		assert.Equal(t, float64(inputs[4]), p2.DistanceTo(p1))
	}
}

func FuzzDistanceTo(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, x1 int64, y1 int64, x2 int64, y2 int64) {
		...
		assert.InDelta(t, expected, p1.DistanceTo(p2), .001)
		assert.InDelta(t, expected, p2.DistanceTo(p1), .001)
